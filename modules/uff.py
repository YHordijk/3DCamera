...
class ForceField:
	...
	def get_energy_from_coords(self, coords, morse_potential=True, use_torsions=True):
		...
		for i, a in enumerate(atoms):
			utils.message(f'{i: <3} | {self.get_atom_type(a): <5} | {a.ring}', 1)
		...
		utils.message('ATOM 1 | ATOM 2 | BO  | BOND LEN | IDEAL LEN | ENERGY', 2)
		...
		for a1, a2 in molecule.get_unique_bonds():
			...
			utils.message(f'{e1: <6} | {e2: <6} | {n: <3.1f} | {r: <8.3f} | {r12: <8.3f} | {E_ri: <.3f}', 2)
		...
		#E_phi:
		utils.message('BOND STRETCH ENERGY', 2)
		utils.message('ATOM 1 | ATOM 2 | ATOM 3 | ATOM 4 | TORSION | IDEAL TORS | ENERGY', 2)
		...
		for a1, a2, a3, a4, phi in molecule.get_unique_torsion_angles():
			...
			utils.message(f'{e1: <6} | {e2: <6} | {e3: <6} | {e4: <6} | {torsion: <7.3f} | {phi0[0]: <10.3f} | {E_phi_i: <.3f}', 2)
		...
		for a1, a2 in molecule.get_unique_atom_pairs(): #cutoff is at 2 angstrom
			if a1.bond_dist_to(a2) > 2:
				...
				utils.message(f'{e1: <6} | {e2: <6} | {x: <8.3f} | {D12: <11.3f} | {E_vdw_i: <.3f}', 2)




		utils.message(f'TOTAL BOND STRETCHING ENERGY = {round(E_r*4.2,3)} kJ/mol',1)
		utils.message(f'TOTAL ANGLE BENDING ENERGY = {round(E_theta*4.2,3)} kJ/mol',1)
		utils.message(f'TOTAL TORSIONAL ENERGY = {round(E_phi*4.2,3)} kJ/mol',1)
		utils.message(f'TOTAL VAN DER WAAL\'S ENERGY = {round(E_vdw*4.2,3)} kJ/mol',1)
		utils.message(f'TOTAL ENERGY = {round((E_r + E_theta + E_phi + E_vdw)*4.2,3)} kJ/mol',1)
